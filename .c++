...
void loop() {
  ...
  // Create a JSON string to send to the server
  ...
  // Check for feeding commands from the server
  http.begin(serverUrl + "/start-feeding");
  int startFeedingResponseCode = http.GET();
  if (startFeedingResponseCode == 200) {
    startFeeding();
  }

  http.begin(serverUrl + "/stop-feeding");
  int stopFeedingResponseCode = http.GET();
  if (stopFeedingResponseCode == 200) {
    stopFeeding();
  }

  // Check for algae control interval from the server
  http.begin(serverUrl + "/interval");
  int intervalResponseCode = http.GET();
  if (intervalResponseCode == 200) {
    int interval = http.getString().toInt();
